...
module encode_132bit_pattern (edgechoice,enable,d,CLK200,CLK400,code,tdc_hit,scaler_hit);
	...
	generate	
		for (i=0; i < 33; i=i+1) begin : STAGE0 	//i = 0,...,32
		
			reg [3:0] pattern;

			always@(posedge CLK400)
			begin
				pattern <= {d[4*i+3:4*i+0]};
			end
	...
	generate	
		for (i=0; i < 33; i=i+1) begin : STAGE1 	//i = 0,...,32
			...
			begin
					...
					if (STAGE0[i].pattern == 4'b1000) b1111 <= 1'b1; else b1111 <= 1'b0;
					if (STAGE0[i].pattern == 4'b0111) b0000 <= 1'b1; else b0000 <= 1'b0;
					...
					case (STAGE0[i].pattern)
						...
						4'b0000: begin addr <= 2'b10; end 
						4'b0001: begin addr <= 2'b10; end 
						4'b0010: begin addr <= 2'b10; end 
						4'b0011: begin addr <= 2'b10; end 

						//3. valid (but only if followed by !b'0000 and 4'b1111)
						4'b0100: begin addr <= 2'b01; end 
						4'b0101: begin addr <= 2'b01; end 

						//4. valid (but only if followed by !b'0000 and 4'b1111)
						4'b0110: begin addr <= 2'b00; end 

						//invalid
						4'b0111: begin addr <= 2'b00; end 
					endcase
			end 
	...
	generate	
		for (i=0; i < 30; i=i+1) begin : STAGE2 	//i = 0,...,29
	...
	//-- 3. stage - mux - first hit wins (30 groups left, divide by two)
	generate	
		for (i=0; i < 15; i=i+1) begin : STAGE3  //i = 0,...,14
	...
	//-- 4. stage - mux - first hit wins (15 groups left, cannot simply divide by two)
	generate	
		for (i=0; i < 8; i=i+1) begin : STAGE4  //i = 0,...,7
			...
			if (i==7) begin
					...
					always@(posedge CLK400)
					begin	
						valid <= STAGE3[14].valid;
						addr <= {1'b0,STAGE3[14].addr}; 
					end
	...
	//-- 5. stage - mux - first hit wins (8 groups left, divide by two)
	generate	
